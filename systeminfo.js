...
const scheds = {};
const states = {};
const roleNames = ['number', 'switch', 'boolean', 'value.temperature', 'json', 'string'];
const roleRoles = ['value', 'switch', 'value', 'value.temperature', 'value', 'value'];
const roleTypes = ['number', 'boolean', 'boolean', 'number', 'string', 'string'];
const reIsEvalWrite = /@\((.+)\)/;
const reIsMultiName = /^([^\s,\[]+)\s*(\[\s*(\w+\s*\/\s*\w*|[^\s,\]]+(\s*\,\s*[^\s,\]]+)+|\*)\s*\]\s*)?(\S*)$/;
const reIsInfoName = /^(\w*)\s*(\(\s*([^\(\),\s]+)\s*(\,\s*\S+\s*)*\))?$/;
const reIsRegExp = /^\/(.*)\/([gimy])*$/;
const reIsObjName = /\s*(\w+)\s*\/\s*(\w*)\s*/;
const reStripPrefix = /(?!xmlns)^.*:/;
const reIsSchedule = /^[\d\-\/\*\,]+(\s+[\d\/\-\*,]+){4,5}$/;
...
function xmlParseString(body) {
    const valp = (str /* , name */) =>
            !isNaN(str) ? A.number(str) : /^(?:true|false)$/i.test(str) ? str.toLowerCase() === 'true' : str,
        options = {
            explicitArray: false,
            explicitRoot: false,
            //			ignoreAttrs: true,
...
class Cache {
    constructor(fun) {
        // neue Einträge werden mit dieser Funktion kreiert
...
class WebQuery {
    ...
    static eval(fun, _v, con) {
        try {
            const res = eval(fun.replace(/@/g, '_v')); // A.D(`eval:[${x}] returns ${res} and had ${A.O(_v)}`);
            return res;
        } catch (e) {
...
function main() {
    ...
    for (const item of adapter.config.items) {
        ...
        if (ir[2]) {
            ...
            if (irn.mid !== '*') {
                const on = irn.mid.match(reIsObjName);
                if (on) {
                    irn.name = on[1];
        ...
        const ra = A.trim(A.T(ni.role, '') ? ni.role.split('|') : 'value'),
    ...
    A.seriesOf(
        ...
        .then(() =>
            A.getObjectList({
                ...
                .then(() =>
                    A.I(
                        `Adapter ${A.ains} started and found ${A.obToArray(list).reduce(
